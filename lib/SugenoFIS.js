...
class SugenoFIS {
  constructor(rules = []) {
    this._linguisticVariableMSF = new Map()
    this._rules = rules
    this._ruleText = new Set()
  }

  setLinguisticVariable(name, msf) {
    this._linguisticVariableMSF.set(name, msf)
  }
  ...
  addRule(ruleText) {
    ...
    antecedent.split(' and ').forEach(expression => {
      ...
      if (!this._linguisticVariableMSF.has(linguisticVariableName)) {
    ...
    this._rules.push(new SugenoRule({
      expressions,
      outputMsf,
      msfRegistry: this._linguisticVariableMSF,
      ruleText
    }))
  }
  ...
  valueOf(setName, x) {
    return this._linguisticVariableMSF.get(setName)(x)
  }
